...
export async function POST(request: Request) {
  ...
  if (!process.env.RESEND_API_KEY || !process.env.BROZZ_EMAIL_FROM || !process.env.BROZZ_EMAIL_TO) return NextResponse.json({ message: "Email delivery is not configured yet. Please email hello@brozzedits.com directly." }, { status: 503 });
